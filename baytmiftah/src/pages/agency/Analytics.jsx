...
const metrics = [
  ['Conversion Rate', '4.82%', '+0.4% this month', 'trending_up', 'text-[#E9C349]'],
  ['Avg. Time to Close', '18 Days', 'Down from 22 days', 'schedule', 'text-red-600'],
  ['Active Campaigns', '7', '3 featured placements', 'campaign', 'text-[#E9C349]'],
]

const forecast = [
  ['New leads', 128, '+18%'],
  ['Viewing requests', 42, '+9%'],
  ['Offers expected', 11, '+4%'],
  ['Projected GCI', 'GHS 182K', '+12%'],
]
        ...
        <section className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div>
              ...
              <p className="mt-3 text-xl text-[#303744]">
                Portfolio: Skyreach International Hub and 12 active assets
              </p>
            </div>
            <div className="rounded-lg border border-[#cbd3df] bg-white px-5 py-4 text-right">
              <p className="font-bold uppercase tracking-widest text-[#E9C349]">Live Status</p>
          ...
          <div className="mt-8 grid gap-7 xl:grid-cols-[minmax(0,1fr)_420px]">
            <article className="rounded-lg border border-[#cbd3df] bg-white p-8">
                ...
                {bars.map((height, index) => (
                  <div
                    ...
                    className={`w-10 rounded-t ${
                      index === 5
                        ? 'bg-gradient-to-b from-black to-[#d6dde8]'
                        : 'bg-gradient-to-b from-[#F5D76B] to-[#eaf6ff]'
                    }`}
                    style={{ height: `${height}%` }}
                  />
                ))}
              ...
              <footer className="mt-6 flex items-center justify-between text-lg font-bold">
                ...
                <span className="text-[#E9C349]">+12.4% vs LY</span>
              </footer>
            </article>

            <div className="grid gap-7">
              {metrics.map(([label, value, subtext, icon, tone]) => (
                <article key={label} className="rounded-lg border border-[#cbd3df] bg-white p-8">
                  ...
                  <p className={`mt-2 text-lg ${tone}`}>{subtext}</p>
                </article>
              ))}
            </div>
          </div>

          <section className="mt-8 grid gap-5 rounded-lg border border-[#cbd3df] bg-white p-8 md:grid-cols-4">
            <div className="md:col-span-4">
              <h2 className="text-3xl font-bold">Conversion Forecast</h2>
              <p className="mt-2 text-[#303744]">
                Forward-looking pipeline estimate from current lead, viewing, and offer activity.
              </p>
            </div>
            {forecast.map(([label, value, delta]) => (
              <article key={label} className="rounded-lg bg-[#edf4ff] p-5">
                <p className="text-[#596170]">{label}</p>
                <p className="mt-2 text-3xl font-black">{value}</p>
                <p className="mt-1 font-bold text-[#E9C349]">{delta}</p>
              </article>
            ))}
          </section>
          ...
          <div className="mt-8 grid gap-7 xl:grid-cols-[minmax(0,1fr)_420px]">
            <section className="overflow-hidden rounded-lg border border-[#cbd3df] bg-white">
              <header className="flex flex-col gap-4 p-8 md:flex-row md:items-center md:justify-between">
              ...
              {leads.map((lead) => (
                <div
                  key={lead[1]}
                  className="grid min-w-[820px] grid-cols-[1.3fr_1fr_1fr_1fr_0.7fr] items-center border-t border-[#d8dde6] px-8 py-7"
                >
            ...
            <aside className="space-y-7">
              <article className="rounded-lg border border-[#E9C349] bg-[#eefcf8] p-8">
                <h2 className="flex items-center gap-4 text-3xl font-bold">
                  <span className="material-symbols-outlined text-[#E9C349]">rocket_launch</span>
                  Marketing Boost
                </h2>
                <p className="mt-7 text-lg leading-7 text-[#303744]">
                  Increase listing visibility with featured placements, retargeting, and campaign credits.
                </p>
                {[
                  ...
                  <div key={title} className="mt-5 flex items-center justify-between rounded-md bg-white/70 p-5">
                    ...
                    <span className={`h-8 w-14 rounded-full ${enabled ? 'bg-[#E9C349]' : 'bg-[#cbd3df]'}`} />
                  </div>
                ))}
                <div className="mt-8">
                  ...
                  <div className="h-2 rounded-full bg-[#cbd3df]">
                    <div className="h-2 w-[85%] rounded-full bg-[#E9C349]" />
                  </div>
                </div>
                <button className="mt-8 w-full rounded-lg border-2 border-[#E9C349] py-5 text-xl font-bold text-[#E9C349]">
                  Manage Placements
                </button>
              </article>
              ...
              <article className="rounded-lg bg-[#1d334a] p-8 text-white shadow-xl">
                ...
                <p className="mt-6 text-6xl font-black">
                  2.4k <span className="text-xl text-[#F5D76B]">+18%</span>
                </p>
                <div className="mt-10 flex h-28 items-end gap-3">
                  {[20, 36, 55, 46, 72, 90].map((height) => (
                    <span
                      key={height}
                      className="flex-1 rounded-t bg-[#E9C349]"
                      style={{ height: `${height}%` }}
                    />
                  ))}
                </div>
          ...
          <section className="mt-8 overflow-hidden rounded-lg border border-[#cbd3df] bg-white">
            <header className="flex flex-col gap-2 p-8 md:flex-row md:items-center md:justify-between">
          ...
          <section className="mt-8 overflow-hidden rounded-lg border border-[#cbd3df] bg-white">
            ...
            {agents.map((row) => (
              <div
                key={row[0]}
                className="grid min-w-[900px] grid-cols-[1.2fr_1fr_1fr_1fr_1fr] items-center border-t border-[#d8dde6] px-8 py-6 text-xl"
              >
                ...
                <span>
                  <span className="inline-block h-2 w-32 rounded-full bg-[#edf4ff] align-middle">
                    <span className="block h-2 rounded-full bg-[#E9C349]" style={{ width: row[3] }} />
                  </span>
                </span>
                <strong className="text-[#E9C349]">{row[4]}</strong>
              </div>
            ))}
          </section>
        </section>
