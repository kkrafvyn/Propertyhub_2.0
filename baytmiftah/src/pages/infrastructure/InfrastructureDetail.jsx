...
import { getInfrastructureModule } from '../../data/infrastructure-modules'

export default function InfrastructureDetail() {
  const { moduleId } = useParams()
  const module = getInfrastructureModule(moduleId)

  if (!module) return <Navigate to="/infrastructure" replace />
        ...
        <div className="mx-auto max-w-container px-4 pt-24 md:px-8">
          <section className="rounded-lg border border-[#cbd3df] bg-white p-6 shadow-sm md:p-8">
            <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
              ...
              <Link to="/infrastructure" className="btn-secondary inline-flex items-center gap-2 self-start">
          ...
          <section className="mt-8 grid gap-5 lg:grid-cols-[1fr_0.8fr]">
            <div className="rounded-lg border border-[#cbd3df] bg-white p-6 shadow-sm">
              <h3 className="text-2xl font-black">Capabilities</h3>
            ...
            <div className="rounded-lg border border-[#cbd3df] bg-white p-6 shadow-sm">
              <h3 className="text-2xl font-black">Strategic outcomes</h3>
              <div className="mt-5 grid gap-3">
                {module.outcomes.map((outcome) => (
                  <div key={outcome} className="rounded-md border border-[#e5e7eb] p-4">
