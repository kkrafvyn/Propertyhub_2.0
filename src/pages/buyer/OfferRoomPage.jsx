    ...
    <DesktopShell search={<CompactSearch />}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mt-6 max-w-lg space-y-3 panel-card bg-surface-subtle p-5">
          ...
          <button type="submit" className="rounded-lg bg-brand-accent px-4 py-2 text-sm font-semibold text-white">Submit offer</button>
        </form>
      )}

      <div className="mt-8 space-y-3">
        {offers.map((o) => (
          <article key={o.id} className="flex flex-wrap items-center justify-between gap-3 panel-card bg-surface p-4">
            <div>
            ...
            <span className="rounded-full bg-surface-hover px-3 py-1 text-xs font-semibold capitalize text-ink">{o.status}</span>
          </article>
        ))}
      </div>
    </DesktopShell>
