...
function Maintenance() {
  ...
  return (
    <RenterShell title="Maintenance requests" subtitle="Report issues and track repairs">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-3 rounded-card border border-surface-border bg-surface-subtle p-5">
          ...
          <button type="submit" className="rounded-lg bg-brand-dark px-4 py-2 text-sm font-semibold text-brand">Submit</button>
        </form>
      )}

      <div className="mt-6 space-y-3">
        {requests.map((r) => (
          <article key={r.id} className="rounded-card border border-surface-border bg-surface p-4">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <p className="font-semibold">{r.title}</p>
              <span className="rounded-full bg-brand-light px-2 py-0.5 text-xs font-semibold capitalize text-brand-dark">{r.status.replace('_', ' ')}</span>
            </div>
