...
function Market() {
  ...
  return (
    <IntelligenceShell title="Market intelligence" subtitle={summary?.region || 'Greater Accra'}>
      {summary && (
        <div className="mb-6 flex flex-wrap gap-2">
          {summary.hotZones?.map((z) => (
            <span key={z} className="rounded-full bg-brand-light px-3 py-1 text-xs font-semibold text-brand-dark">{z}</span>
          ))}
        </div>
      )}
          ...
          {trends.map((t) => (
            <div key={t.month} className="flex items-center gap-3">
              ...
              <div className="h-3 flex-1 overflow-hidden rounded-full bg-surface-subtle">
                <div className="h-full rounded-full bg-brand-dark" style={{ width: `${(t.median / 3000000) * 100}%` }} />
              </div>
