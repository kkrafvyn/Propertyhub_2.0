...
export default function OAuthButtons({
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <GoogleIcon />
        {loadingProvider === 'google' ? t('auth.redirecting') : t('auth.continueGoogle')}
      </button>
      <button
        type="button"
        disabled={disabled || busy}
      ...
      >
        <AppleIcon />
        {loadingProvider === 'apple' ? t('auth.redirecting') : t('auth.continueApple')}
      </button>
    </div>
  )
}
  ...
  return (
    <div className="relative my-6">
      ...
      <div className="relative flex justify-center text-xs uppercase tracking-wide">
        <span className="bg-surface px-3 text-ink-secondary">{t('auth.or')}</span>
      </div>
    </div>
  )
}
