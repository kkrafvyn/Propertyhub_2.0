...
describe("mobileDeepLinkService", () => {
  it("normalizes trusted app and web URLs into internal routes", () => {
    expect(mobileDeepLinkService.toAppPath("propertyhub://property/listing-1?from=push")).toBe(
      "/property/listing-1?from=push"
    );
    expect(mobileDeepLinkService.toAppPath("https://propertyhub.app/app/deals#offer")).toBe(
      "/app/deals#offer"
    );
  ...
  it("falls back to home for unknown hosts or unsupported paths", () => {
    ...
    expect(mobileDeepLinkService.toAppPath("propertyhub://admin/users")).toBe("/");
