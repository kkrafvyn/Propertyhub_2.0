...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: propertyTransaction, error: transactionError } = await admin
      ...
      .select("id, payer_user_id, organization_id, provider, provider_transaction_id")
    ...
    const provider = normalizePaymentGatewayProvider(propertyTransaction.provider);
    const verificationReference =
      provider === "stripe" && propertyTransaction.provider_transaction_id
        ? propertyTransaction.provider_transaction_id
        : reference;
    const verifiedTransaction = await verifyGatewayTransaction(provider, verificationReference);
