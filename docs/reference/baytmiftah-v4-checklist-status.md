...
| IoT rollout pack | `docs/iot/SMART_PROPERTY_ACCESS_ROLLOUT.md` now defines the TTLock-first/Tuya-second pilot, buying list, provider account setup, expanded IoT env placeholders, live-device test matrix, property-type coverage, operating rules, and go/no-go criteria. |
| API-key-free completion lab | `/innovation-lab` now exposes the remaining moat features without secrets: CMS/newsletter capture, creator marketplace controls, rich media studio, live open-house readiness, tax and holding-cost simulator, hyperlocal signal board, moderated community/contributor flows, affordability plan simulator, payment/identity/IoT provider activation matrix, and no-key helper logic/tests. |
| Provider-ready callable functions | `provider-ready-features.service.ts` now provides actual callable functions for editorial drafts, newsletter subscriptions, live open houses, measured floor plans, SMS/USSD/WhatsApp requests, community spaces/contributions, manual identity/registry checks, advanced fraud signals, referral rewards, affordability plans, hyperlocal sources, construction progress updates, contributor payouts, payment fallback planning, and wallet checkout readiness. |
| Provider-ready operation functions | Moderation, status updates, newsletter campaigns, open-house registrations, community reports, fraud-review decisions, referral payout decisions, contributor profile reviews, construction review, hyperlocal import logging, and feature operation events now have callable service functions and migration-backed tables without requiring provider API keys. |
| Production activation depth | `production-activation.service.ts` and the production activation migration now wire provider integration runs, legal signoffs, hyperlocal observations, community broadcasts/chats, affordability plan enrollments, human-reviewed investment score reviews, contributor marketplace listings, construction forecasts, developer verification, white-label packages, and commission rules without storing API keys. Weekly audit anchoring is wired through `.github/workflows/audit-anchor.yml` and will run after `SUPABASE_PROJECT_REF` and `ANCHOR_JOB_SECRET` are configured in GitHub secrets. |
...
| Scheduled anchoring | Configure `SUPABASE_PROJECT_REF` and `ANCHOR_JOB_SECRET` in GitHub Actions secrets, then monitor `.github/workflows/audit-anchor.yml` weekly runs. |
...
| Competitive moat integrations | App-side readiness, review records, price-drop alerts, finance widgets, rich media, smart compare, agency trust score, WhatsApp readiness, Ghana Card/manual registry readiness, owner reporting readiness, document templates, calendar export readiness, community prompts, human-reviewed fraud signals, investment preview, affordability guardrails, construction readiness, contributor monetization preview, `/innovation-lab` no-key demo surfaces, callable no-key feature functions, provider activation runs, hyperlocal observations, community broadcasts/chats, plan enrollments, investment review, contributor marketplace, construction forecasts, white-label packaging, and commission controls are wired. Add production providers/data sources and legal approvals before marking them live. |
